...
describe('memory', () => {
  ...
  it('storeBytes and loadBytes round-trip', () => {
    ...
    storeBytes({ memory, address: 8, source: eight, byteLength: 8 });

    const destination = new Uint8Array(8);
    loadBytes({ destination, memory, address: 8, byteLength: 8 });
  ...
  it('loadBytes zero-extends into the destination high bytes', () => {
    ...
    loadBytes({ destination, memory, address: 4, byteLength: 1 });
