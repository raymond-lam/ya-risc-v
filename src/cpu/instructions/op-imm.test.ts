...
describe('op-imm', () => {
  it('addi writes rs1 + imm and advances pc', () => {
    ...
    addi(registers, createMemory(256), {
      ...
      sourceRegister1: 0,
      immediate: signedNumberToBytes(10, 32),
    });
    assert.deepEqual(readGeneralPurposeRegister(registers, 1), signedNumberToBytes(10, 32));
  ...
  it('slti and sltiu compare signed vs unsigned', () => {
    ...
    slti(registers, createMemory(256), {
      ...
      sourceRegister1: 1,
      immediate: signedNumberToBytes(1, 32),
    });
  ...
  it('xori bitwise-xors the immediate', () => {
    const registers = createRegisters();
    writeGeneralPurposeRegister(registers, 1, signedNumberToBytes(0x0f, 32));
    xori(registers, createMemory(256), {
      destinationRegister: 2,
      sourceRegister1: 1,
      immediate: signedNumberToBytes(0xff, 32),
    });
    assert.deepEqual(readGeneralPurposeRegister(registers, 2), signedNumberToBytes(0xf0, 32));
  });
});
